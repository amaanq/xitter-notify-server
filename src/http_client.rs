...
impl Display for HttpError {
   fn fmt(&self, f: &mut Formatter<'_>) -> FmtResult {
      match *self {
         Self::Request(ref error) => write!(f, "request error: {error}"),
         Self::Status(code, ref body) => write!(f, "HTTP {code}: {body}"),
         Self::Body(ref error) => write!(f, "body error: {error}"),
      }
   }
}
...
impl HttpClient {
   ...
   pub async fn get<H>(&self, url: &str, headers: &[(&str, H)]) -> Result<Vec<u8>, HttpError>
   where
      H: AsRef<str> + Sync,
   {
      let mut builder = Request::builder().method(Method::GET).uri(url);

      for &(key, ref value) in headers {
   ...
   pub async fn get_text(&self, url: &str) -> Result<String, HttpError> {
      ...
      String::from_utf8(body).map_err(|error| HttpError::Body(error.to_string()))
   }
