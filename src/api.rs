...
impl StatusResponse {
   ...
   const fn ok_with_users(users: i64) -> Self {
      Self {
         status: "ok",
         users:  Some(users),
         error:  None,
...
async fn register(
   ...
   if request.twitter_user_id.is_empty() {
      return (
   ...
   if request.csrf_token.is_empty() {
      return (
         StatusCode::BAD_REQUEST,
         Json(StatusResponse::error("csrf_token is required")),
      );
   }
   if request.up_endpoint.is_empty() {
      return (
         StatusCode::BAD_REQUEST,
   ...
   if !request.up_endpoint.starts_with("https://") && !request.up_endpoint.starts_with("http://") {
      return (
         StatusCode::BAD_REQUEST,
   ...
   match state.db.register_user(
      &request.twitter_user_id,
      &request.auth_token,
      &request.csrf_token,
      &request.up_endpoint,
   ) {
      Ok(_) => {
         eprintln!("[api] Registered user {}", request.twitter_user_id);
         (StatusCode::OK, Json(StatusResponse::ok()))
      },
      Err(error) => {
         eprintln!("[api] Failed to register user: {error}");
         (
...
async fn unregister(
   ...
   match state.db.unregister_user(&request.twitter_user_id) {
      Ok(deleted) => {
         if deleted {
            eprintln!("[api] Unregistered user {}", request.twitter_user_id);
            (StatusCode::OK, Json(StatusResponse::ok()))
         } else {
            (
      ...
      Err(error) => {
         eprintln!("[api] Failed to unregister user: {error}");
         (
            StatusCode::INTERNAL_SERVER_ERROR,
...
async fn health(State(state): State<Arc<AppState>>) -> impl IntoResponse {
   match state.db.user_count() {
      ...
      Err(error) => {
         eprintln!("[api] Health check failed: {error}");
         (
            StatusCode::INTERNAL_SERVER_ERROR,
...
async fn generate_txid(
   ...
   if query.force
      && let Err(error) = state.txid_generator.invalidate_and_refresh().await
   {
      eprintln!("[api] Failed to force refresh: {error}");
   }

   match state.txid_generator.generate("GET", &query.path).await {
      Ok(txid) => (StatusCode::OK, Json(TxIdResponse { txid })).into_response(),
      Err(error) => {
         eprintln!("[api] Failed to generate transaction ID: {error}");
         (
            StatusCode::INTERNAL_SERVER_ERROR,
